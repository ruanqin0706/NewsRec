...
class Manager():
    ...
    def __init__(self, config=None, command=None):
        ...
        parser.add_argument("-s", "--scale", dest="scale", help="data scale", type=str,
        ...
        parser.add_argument("-d", "--device", dest="device", help="gpu index, -1 for cpu",
                            type=lambda x: int(x) if x != "cpu" else "cpu", default=0)
        parser.add_argument("-bs", "--batch-size", dest="batch_size", help="batch size in training", type=int,
                            default=32)
        parser.add_argument("-bse", "--batch-size-eval", dest="batch_size_eval", help="batch size in encoding",
        ...
        parser.add_argument("-ck", "--checkpoint", dest="checkpoint",
                            help="load the model from checkpoint before training/evaluating", type=str, default="none")
        parser.add_argument("-vs", "--validate-step", dest="validate_step",
                            help="evaluate and save the model every step", type=str, default="0")
        parser.add_argument("-hst", "--hold-step", dest="hold_step", help="don't evaluate until reaching hold step",
                            type=str, default="0")
        parser.add_argument("-sav", "--save-at-validate", dest="save_at_validate",
                            help="save the model every time of validating", action="store_true", default=False)
        parser.add_argument("-vb", "--verbose", dest="verbose", help="variant's name", type=str, default=None)
        parser.add_argument("--metrics", dest="metrics", help="metrics for evaluating the model", nargs="+",
                            action="extend", default=["auc", "mean_mrr", "ndcg@5", "ndcg@10"])

        parser.add_argument("-hs", "--his_size", dest="his_size", help="history size", type=int, default=50)
        parser.add_argument("-is", "--impr_size", dest="impr_size", help="impression size for evaluating", type=int,
                            default=20)
        parser.add_argument("-nn", "--negative-num", dest="negative_num", help="number of negatives", type=int,
                            default=4)
        parser.add_argument("-dp", "--dropout-p", dest="dropout_p", help="dropout probability", type=float, default=0.1)
        parser.add_argument("-lr", "--learning-rate", dest="learning_rate", help="learning rate", type=float,
                            default=1e-5)
        parser.add_argument("-sch", "--scheduler", dest="scheduler", help="choose schedule scheme for optimizer",
                            choices=["linear", "none"], default="none")
        parser.add_argument("--warmup", dest="warmup", help="warmup steps of scheduler", type=float, default=0.1)

        parser.add_argument("-pth", "--preprocess-threads", dest="preprocess_threads",
                            help="thread number in preprocessing", type=int, default=32)
        parser.add_argument("-dr", "--data-root", dest="data_root", default="../../../Data")
        parser.add_argument("-id", "--infer-dir", dest="infer_dir", default="../../../Data")
        ...
        parser.add_argument("-ef", "--enable-fields", dest="enable_fields", help="text fields to model", nargs="+",
                            action="extend", choices=["title", "abs"], default=[])
        ...
        parser.add_argument("-hn", "--head-num", dest="head_num", help="attention head number of tranformer model",
                            type=int, default=12)

        parser.add_argument("-k", dest="k", help="gate number", type=int, default=4)

        parser.add_argument("-plm", dest="plm", help="short name of pre-trained language models", type=str,
                            default="bert")
        ...
        self.exclude_hparams = set(
            ["news_nums", "vocab_size_map", "metrics", "plm_dim", "plm_dir", "data_root", "cache_root", "distributed",
             "exclude_hparams", "rank", "epochs", "mode", "debug", "special_token_ids", "validate_step", "hold_step",
             "exclude_hparams", "device", "save_at_validate", "preprocess_threads", "base_rank", "max_title_length",
             "max_abs_length"])

        logger.info("Hyper Parameters are:\n{}\n".format(
            {k: v for k, v in args.items() if k[0:2] != "__" and k not in self.exclude_hparams}))
    ...
    def prepare(self):
        ...
        if "train" in self.dataloaders:
            ...
            if self.distributed:
                sampler_train = DistributedSampler(dataset_train, num_replicas=self.world_size, rank=self.rank,
                                                   seed=self.seed)
                shuffle = False
            else:
                sampler_train = None
                shuffle = True
            loaders["train"] = DataLoader(dataset_train, batch_size=self.batch_size, shuffle=shuffle,
        ...
        if "dev" in self.dataloaders:
            ...
            loaders["dev"] = DataLoader(dataset_dev, batch_size=self.batch_size_eval, sampler=sampler_dev,
                                        drop_last=False)
        ...
        if "test" in self.dataloaders:
            ...
            loaders["test"] = DataLoader(dataset_test, batch_size=self.batch_size_eval, sampler=sampler_test,
                                         drop_last=False)
            ...
            for k, v in state_dict["manager"].items():
                try:
                    if v != current_manager[k] and k not in {"dataloaders", "checkpoint"}:
                        logger.info(
                            f"manager settings {k} of the checkpoint is {v}, while it's {current_manager[k]} in current setting!")
                except KeyError:
    ...
    def _train(self, model, loaders, validate_step, hold_step, optimizer, scheduler, save_at_validate=False):
        ...
        if self.rank == 0:
            logger.info("training {}...".format(
                model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name))
        ...
        for epoch in range(self.epochs):
            ...
            if self.distributed:
                try:
                    ...
                    if self.rank == 0:
                        logger.warning(
                            f"{type(loader_train)} has no attribute 'sampler', make sure you're using Triple training dataloader")
            ...
            for step, x in enumerate(tqdm_, 1):
                ...
                if total_steps > hold_step and total_steps % validate_step == 0:
                    ...
                    if self.rank == 0:
                        ...
                        if save_at_validate:
                            self.save(model, total_steps, best=False)
                        ...
                        if result["main"] >= best_res["main"]:
                            ...
                            self.save(model, total_steps, best=False)
                            self._log(model.module.name if isinstance(model,
                                                                      torch.nn.parallel.DistributedDataParallel) else model.name,
                                      result)
    ...
    def train(self, model, loaders):
        ...
        if self.rank == 0:
            ...
            os.makedirs(
                f"data/ckpts/{model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name}/{self.scale}",
                exist_ok=True)
        ...
        result = self._train(model, loaders, validate_step, hold_step, optimizer, scheduler=scheduler,
                             save_at_validate=self.save_at_validate)

        if self.rank in [-1, 0]:
            logger.info("Best result: {}".format(result))
            self._log(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name,
                      result)
