...
class DKNTextIterator(BaseIterator):
    ...
    def parser_one_line(self, line):
        ...
        if candidate_news not in self.news_word_index:
            candidate_news_index = self.news_word_index[list(self.news_word_index.keys())[0]]
        else:
            candidate_news_index = self.news_word_index[candidate_news]
        if candidate_news not in self.news_entity_index:
            candidate_news_entity_index = self.news_entity_index[list(self.news_entity_index.keys())[0]]
        else:
            candidate_news_entity_index = self.news_entity_index[candidate_news]

        if userid not in self.user_history:
            click_news_index = self.user_history[list(self.user_history.keys())[0]][0]
            click_news_entity_index = self.user_history[list(self.user_history.keys())[0]][1]
        else:
