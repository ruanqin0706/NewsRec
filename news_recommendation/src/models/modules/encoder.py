...
class CnnNewsEncoder(BaseNewsEncoder):
    ...
    def forward(self, token_id, attn_mask, token_weight=None):
        ...
        news_embedding = scaled_dp_attention(self.news_query, self.Tanh(self.newsProject(cnn_output)), cnn_output,
                                             attn_mask=attn_mask.unsqueeze(-2)).squeeze(dim=-2)
...
class GatedBertNewsEncoder(BaseNewsEncoder):
    ...
    def forward(self, token_id, attn_mask, token_weight=None):
        ...
        news_embedding = scaled_dp_attention(self.news_query, token_embedding, token_embedding,
                                             attn_mask=attn_mask.unsqueeze(-2)).squeeze(dim=-2).view(
            *original_shape[:-1], -1)
        token_embedding = token_embedding.view(*original_shape, -1)
        return token_embedding, news_embedding
...
class TfmNewsEncoder(BaseNewsEncoder):
    ...
    def forward(self, token_id, attn_mask, token_weight=None):
        ...
        news_embedding = scaled_dp_attention(self.news_query, self.Tanh(self.newsProject(token_embedding)),
                                             token_embedding, attn_mask=attn_mask.unsqueeze(-2)).squeeze(dim=-2).view(
            *original_shape[:-1], -1)
        token_embedding = token_embedding.view(*original_shape, -1)
        return token_embedding, news_embedding
...
class TfmUserEncoder(BaseUserEncoder):
    ...
    def forward(self, news_embedding, his_mask):
        ...
        user_embedding = scaled_dp_attention(self.user_query, self.Tanh(self.userProject(news_embedding)),
                                             news_embedding, attn_mask=his_mask.unsqueeze(-2))
        return user_embedding
