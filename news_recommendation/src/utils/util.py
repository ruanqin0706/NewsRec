...
def pack_results(impr_indices, masks, *associated_lists):
    ...
    for x in tqdm(zip(impr_indices, masks, *associated_lists), total=len(impr_indices), desc="Packing Results",
                  ncols=80):
        key = x[0]
        mask = x[1]
        values = x[2:]
...
def construct_uid2index(data_root, cache_root):
    ...
    uid2index = {}
    user_df_list = []
    behaviors_file_list = [os.path.join(data_root, "MIND", directory, "behaviors.tsv") for directory in
    ...
    def __call__(self, documents):
        ...
        for tf, document in zip(self.tfs, documents):
            ...
            for word, freq in tf.items():
                ...
                if len(word) == 1:
                    continue
                score = (self.idf[word] * freq * (self.k + 1)) / (
                            freq + self.k * (1 - self.b + self.b * len(document) / self.doc_avg_length))
